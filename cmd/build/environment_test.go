...
func TestInitBuildServiceFailsFastOnUndeclaredEnvironment(t *testing.T) {
	ctx := context.Background()
	dir := t.TempDir()
	files := map[string]string{
		"workspace.codefly.yaml": `name: build-env
layout: modules
modules:
    - name: web
`,
...
`,
	...
	flow, err := initBuildService(ctx, workspace, module, service, true)
	require.Nil(t, flow)
	require.Error(t, err)
	require.Contains(t, err.Error(), `"production"`)
	require.Contains(t, err.Error(), "build-env")
}
