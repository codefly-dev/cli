...
func TestTargetCommandHasOfflineHelpAndRequiresExplicitEnvironment(t *testing.T) {
	...
	workspace := t.TempDir()
	data, err := yaml.Marshal(&resources.Workspace{Name: "target-inspection", Layout: "modules", Environments: []*resources.Environment{
		{Name: "missing-namespace", Cluster: &resources.EnvironmentCluster{Kind: "k3d"}},
		{Name: "remote", Namespace: "product", Cluster: &resources.EnvironmentCluster{Kind: "remote"}},
	}})
	...
	for _, test := range []struct{ environment, message string }{
		{"undeclared", "not declared"}, {"missing-namespace", "explicit valid environment namespace"}, {"remote", "exact local k3d target"},
	} {
		command = NewCommand()
		output.Reset()
		command.SetOut(&output)
