...
// would audit through the candidate instead of the selected packager.
func TestAgentCIAuditsTheSelectedSourcePackager(t *testing.T) {
	binDir := t.TempDir()
	cli := filepath.Join(binDir, "codefly")
	peer := filepath.Join(binDir, "peer")
	for output, source := range map[string]string{cli: "../codefly", peer: "../../pkg/sourceworkspace/testdata/agent"} {
	...
	for _, selection := range []string{"implicit", "older", "latest", "self"} {
		t.Run(selection, func(t *testing.T) {
			root, home, output := t.TempDir(), t.TempDir(), t.TempDir()
			manifest := "publisher: example.test\nkind: codefly:service\nname: candidate\nversion: 9.0.0\n"
			selected := "packager__1.0.0"
			switch selection {
			case "older":
			...
			defer cancel()
			command := exec.CommandContext(ctx, cli, "agent", "ci", "--dir", root, "--native-only", "--skip-conformance", "--output", output, "--format", "json")
			command.Env = agentCIChildEnvironment(home, "TEST_SOURCE_CI_RECORD="+calls, "CI_SOURCE_PEER="+peer, "TEST_SOURCE_PACKAGE_EXECUTABLE="+peer,
				"CODEFLY_AGENT_SOURCE=local", "AGENT_REGISTRY=", "AGENT_NIX_FLAKE=", "HTTPS_PROXY=http://127.0.0.1:1", "HTTP_PROXY=http://127.0.0.1:1")
			data, err = command.CombinedOutput()
			require.NoError(t, ctx.Err(), "%s", data)
			var report civ0.AgentCIReport
			require.NoError(t, protojson.Unmarshal(data, &report), "%s", data)
			require.NoError(t, err, "%s", data)
			require.Equal(t, "passed", report.GetStatus(), "%s", data)
			require.Equal(t, "audit", report.Stages[3].GetName())
			require.Equal(t, "passed", report.Stages[3].GetStatus(), "%s", data)
			recorded, err := os.ReadFile(calls)
			require.NoError(t, err)
			lines := strings.Split(strings.TrimSpace(string(recorded)), "\n")
			require.Len(t, lines, 3, "%s", recorded)
			testPath := strings.TrimPrefix(lines[0], "Runtime.Test ")
			auditPath := strings.TrimPrefix(lines[2], "Builder.Audit ")
			require.Equal(t, testPath, strings.TrimPrefix(lines[1], "Builder.Package "))
