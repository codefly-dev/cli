...
func (s *Instance) LoadFactory() error {
	...
	factory, err := LoadFactory(s.Configuration)
	if err != nil {
...
func (s *Instance) LoadRuntime() error {
	...
	runtime, err := LoadRuntime(s.Configuration)
	if err != nil {
		return logger.Wrapf(err, "cannot load factory")
	}
	s.Runtime = runtime
...
func (s *Instance) FactoryInit(r *v1.InitRequest) (*factoryv1.InitResponse, error) {
...
func (s *Instance) Create(r *factoryv1.CreateRequest) (*factoryv1.CreateResponse, error) {
	if server, ok := s.CommunicationServerManager.RequiresCommunication(r); ok {
		...
		s.Logger.Debugf("starting CREATE communication to fetch the information for the plugin")
...
func (s *Instance) Sync(r *factoryv1.SyncRequest) (*factoryv1.SyncResponse, error) {
	if server, ok := s.CommunicationServerManager.RequiresCommunication(r); ok {
		handler := &cli.CliHandler{}
		s.Logger.Debugf("starting SYNC communication to fetch the information for the plugin")
		var answer *corev1.Answer
...
func (s *Instance) RuntimeInit(r *v1.InitRequest) (*runtimev1.InitResponse, error) {
