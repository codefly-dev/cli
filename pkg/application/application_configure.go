...
func (app *Application) Configure(ctx context.Context) error {
	w := wool.Get(ctx).In("applications.Configure<%s>", app.Configuration.Name)
	if app.IsConfigured {
		return nil
	}
...
func (app *Application) ConfigureService(ctx context.Context, instance *services.Instance) error {
	w := wool.Get(ctx).In("applications.ConfigureService<%s::%s[%s]>", app.Configuration.Name, instance.Configuration.Name, instance.Configuration.Agent.Identifier)
