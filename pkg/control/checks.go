...
func (p *planeImpl) runCheckFlow(ctx context.Context, mode orchestration.Mode, req CheckRequest) (CheckResult, error) {
	flow, err := p.buildFlow(ctx, mode, req.Service, orchestration.LocalEnvironmentName, func(_ *resources.Workspace, _ *resources.Environment, _ *resources.Module, _ *resources.Service, f *orchestration.Flow) error {
