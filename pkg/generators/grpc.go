...
func GRPC(ctx context.Context, workspace *resources.Workspace, module *resources.Module, service *resources.Service, language languages.Language, destination string) error {
	w := wool.Get(ctx).In("generateGRPCs", wool.ThisField(resources.WithUnique(service)))
	endpoints, err := getGRPCEndpoints(ctx, workspace, module, service)
	if err != nil {
