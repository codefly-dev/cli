...
func NewRuntimeTestPolicy(ctx context.Context, dependencies *architecture.ServiceDependencies, changeManager ExecutorManager) (*RuntimeTestPolicy, error) {
	return &RuntimeTestPolicy{dependencies: dependencies, ExecutorManager: changeManager}, nil
}
...
func (policy *RuntimeTestPolicy) Execute(ctx context.Context, action Action) ([]Action, error) {
	...
	case RuntimeInit:
		return policy.BasicNext(ctx, outputProperty, action, RuntimeTest)
	case RuntimeTest:
		// We are good
		return nil, nil
	default:
