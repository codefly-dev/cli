...
/*
Runner is a wrapper around a service instance to fit the Change interface
...
type Runner struct {
	...
	// Change managers
	changeForLoad *RunnerLoadManager
	changeForInit *RunnerInitManager
	world         *World
}

func NewRunner(ctx context.Context, instance *services.Instance, world *World, actions chan Action) (*Runner, error) {
	...
	runner := &Runner{
		...
		world:         world,
		actions:       actions,
		changeForLoad: NewRunnerLoadManager(instance.Unique()),
		changeForInit: NewRunnerInitManager(instance.Unique()),
	}
	return runner, nil
}
...
func (runner *Runner) Load(ctx context.Context) (*OutputProperty, error) {
	...
	err = runner.changeForLoad.Set(ctx, &RunnerLoadOutput{Endpoints: resp.Endpoints})
	if err != nil {
		return nil, w.Wrapf(err, "cannot set change for load")
	}
	change, err := runner.changeForLoad.Process(ctx)
	if err != nil {
		return nil, w.Wrapf(err, "cannot process change for load")
	}
	w.Debug("change", wool.Field("change", change))
	return change, nil
}
...
func (runner *Runner) Init(ctx context.Context) (*OutputProperty, error) {
	...
	for _, req := range runner.instance.ProviderDependencies {
		...
		if err != nil {
			return nil, w.Wrapf(err, "cannot get provider information", wool.NameField(req))
		}
	...
	err = runner.changeForInit.Set(ctx, &RunnerInitOutput{})
	if err != nil {
		return nil, w.Wrapf(err, "cannot set change for load")
	}

	change, err := runner.changeForInit.Process(ctx)
	if err != nil {
		return nil, w.Wrapf(err, "cannot process change for load")
	}
